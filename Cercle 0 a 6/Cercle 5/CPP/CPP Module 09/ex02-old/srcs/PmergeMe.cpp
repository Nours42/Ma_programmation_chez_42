...
/*   Updated: 2023/10/22 09:34:07 by nours42          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PmergeMe::_extractStraggler( std::vector<int> &unsortedVector )
{
	...
	if (VERBOSE)
	{
		std::cout << CYAN << std::setw( 35 ) << std::left << "Straggler detected... " << "[" << _straggler << "]" << std::endl;
		_printVector( *_unsortedVector, "Unsorted", PURPLE );
	}
}
