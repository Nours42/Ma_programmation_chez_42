...
/*   Updated: 2023/10/23 13:35:23 by sdestann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	this->_exchangeRate = ref._exchangeRate;
}
...
void BitcoinExchange::_fillExchangeRateEntry(std::string line)
{
	...
	_checkDateString(split[0]);
	_checkValueString(split[1]);

	time_t date = _getEpochFromDateString(split[0]);
	double value = _getValueFromString(split[1]);
