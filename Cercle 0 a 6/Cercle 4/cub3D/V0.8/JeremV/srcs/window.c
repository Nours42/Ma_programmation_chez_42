...
/*   Updated: 2023/09/25 15:23:19 by jmetezea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move_back(t_data *data)
{
	...
	// if (data->orient < 90 && data->orient > 0)
...
void	ft_init_first(t_data *data)
{
	...
	if (data->orient > 0 && data->orient < 90)
	{
		...
		data->first_ya = sqrt((data->first_y * data->first_y) - ((100 - data->player_y
			% PXL) * (100 - data->player_y % PXL)));
	...
	else if (data->orient > -90 && data->orient < 0)
	{
		...
		data->first_xa = sqrt((data->first_x * data->first_x) - ((100 - data->player_x
			% PXL) * (100 - data->player_x % PXL)));
		data->first_y = data->delta_y * (100 - (data->player_y % PXL)) / PXL;
		data->first_ya = sqrt((data->first_y * data->first_y) - ((100 - data->player_y
			% PXL) * (100 - data->player_y % PXL)));
		data->dist_x = data->first_x;
